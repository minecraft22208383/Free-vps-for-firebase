...
mkdir windows
cd windows
wget https://github.com/L36D/Tiny10/releases/download/v1/Tiny10.iso
qemu-img create -f raw windows.img 11G
cd
rm -rf .bashrc
exit
EOF
...
cd /opt/windows
qemu-system-x86_64 -m 2G   -smp 4   -cpu max  -accel tcg,thread=multi -hda windows.img -cdrom Tiny10.iso -device e1000,netdev=net0 -netdev user,id=net0  -vnc :1    
EOF
